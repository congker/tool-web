#!/bin/bash
# 发布 server 下应用，运行的jar从god-server-base任务copy，需先执行god-server-base任务生成jar。
OPTION=$1
PROJECT=$2
PROFILE=$3
MONITOR_PORT=$4
...
copy() {
    ...
    if [[ "${PROFILE}" != "test" ]]; then
        ...
        cp ${PROJECT_HOME}/source/${JAR_NAME} ${APP_DIR}
        echo "[COPY] ${PROJECT_HOME}/source/${JAR_NAME} to ${APP_DIR}"
    fi
}
